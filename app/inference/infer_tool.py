    ...
    def __init__(
        ...
        if not self.only_diffusion:
            ...
            self.unit_interpolate_mode = (
                self.hps_ms.data.unit_interpolate_mode if self.hps_ms.data.unit_interpolate_mode is not None else "left"
            )
            self.vol_embedding = (
                self.hps_ms.model.vol_embedding if self.hps_ms.model.vol_embedding is not None else False
            )
            self.speech_encoder = (
                self.hps_ms.model.speech_encoder if self.hps_ms.model.speech_encoder is not None else "vec768l12"
            )
        ...
        if self.shallow_diffusion or self.only_diffusion:
            if os.path.exists(diffusion_model_path) and os.path.exists(diffusion_model_path):
                self.diffusion_model, self.vocoder, self.diffusion_args = load_model_vocoder(
                    diffusion_model_path, self.dev, config_path=diffusion_config_path
                )
    ...
    def infer(
        ...
        if spk_mix:
            ...
            c, f0, uv = self.get_unit_f0(
                wav, tran, cluster_infer_ratio, speaker, f0_filter, f0_predictor, cr_threshold=cr_threshold
            )
            n_frames = f0.size(1)
        c = c.to(self.dtype)
        f0 = f0.to(self.dtype)
        ...
        with torch.no_grad():
            ...
            if not self.only_diffusion:
                vol = (
                    self.volume_extractor.extract(torch.FloatTensor(wav).to(self.dev)[None, :])[None, :].to(self.dev)
                    if self.vol_embedding
                    else None
                )
                audio, f0 = self.net_g_ms.infer(
                    c, f0=f0, g=sid, uv=uv, predict_f0=auto_predict_f0, noice_scale=noice_scale, vol=vol
                )
            ...
            if self.only_diffusion or self.shallow_diffusion:
                vol = (
                    self.volume_extractor.extract(audio[None, :])[None, :, None].to(self.dev)
                    if vol is None
                    else vol[:, :, None]
                )
