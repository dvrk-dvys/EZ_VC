...
class UpSample1d(nn.Module):
    def __init__(self, ratio=2, kernel_size=None, C=None):
        ...
        if C is not None:
            self.conv_transpose1d_block = [
                nn.ConvTranspose1d(C, C, kernel_size=self.kernel_size, stride=self.stride, groups=C, bias=False)
            ]
...
class DownSample1d(nn.Module):
    def __init__(self, ratio=2, kernel_size=None, C=None):
        ...
        self.lowpass = LowPassFilter1d(
            cutoff=0.5 / ratio, half_width=0.6 / ratio, stride=ratio, kernel_size=self.kernel_size, C=C
        )
